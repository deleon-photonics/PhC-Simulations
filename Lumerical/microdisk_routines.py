...
def waveguide_mode_solver(wg_xspan, wg_yspan, wg_index, wavelength, num_trial_modes = 10, bent_wg = 0, bend_r = 0):
    sim = lp.MODE(hide=True)
    

    fde = so.FDE(xspan = 2*wg_xspan, yspan = 2*wg_yspan, 
                 wavelength = wavelength,
                 ntrials = num_trial_modes, 
                 bent_wg = bent_wg, bend_radius = bend_r)
    fde.add_to_sim(sim)
    ...
    sim.run()
    num_modes = int(sim.findmodes())
    neff = []
    try:
        for i in range(num_modes):
            mode_name = "FDE::data::mode" + str(int(i+1))
            neff.append(np.real(sim.getresult(mode_name, "neff")[0][0]))
        return num_modes, neff
    
    except Exception as e:
        print("An error occurred:", e)
        return -1, [-1]

def microring_resonances_FDE(wg_xspan, wg_yspan, ring_radius, wg_index, wavelength_center, wavelength_span):
    wavelengths = np.linspace(wavelength_center - wavelength_span/2, wavelength_center + wavelength_span/2, int(1 + wavelength_span*1e9 / 0.1))
    nmodes = []
    neffs = []
    for wvl in wavelengths:
        num_mode, neff = waveguide_mode_solver(wg_xspan, wg_yspan, wg_index, wvl, num_trial_modes = 10, bent_wg = 1, bend_r = ring_radius)
        nmodes.append(num_mode)
        neffs.append(neff)
    
    return nmodes, neffs, wavelengths
...
def spoked_microdisk_resonances(outer_radius, width, thickness, material_index, wavelength, wavelength_span, mesh_setting = 1):
    
    sim = lp.FDTD(hide=True)

    output_folder = ("r_" + str(math.ceil(outer_radius*1e9)) + "nm_w_" + 
                         str(math.ceil(width*1e9))  + "nm_t_" + 
                         str(math.ceil((thickness)*1e9)) + "nm")
    ...
    disk = so.microdisk(thickness = thickness, radius = outer_radius, index = material_index)
    disk.add_to_sim(sim)
    air_region = so.microdisk(thickness = thickness, radius = outer_radius - width, index = 1, mesh_order = 1, name = 'air')
    air_region.add_to_sim(sim)
    ...
    if thickness == 0:
        ...
        dimension = '3D'
        dipole_angle = 0
    fdtd = so.FDTD(xspan = outer_radius*2 + 2*wavelength, 
                   yspan = outer_radius*2 + 2*wavelength, 
                   zspan = thickness + wavelength, 
                   dimension = dimension, 
                   sim_time = 20e-12,
                   xmin_bc = 'PML',
                   ymin_bc = 'PML',
    ...
    for xi in [-1,0,1]:
        for yi in [-1,0,1]:
            x = np.cos(theta*np.pi/180)*outer_radius - 3*wavelength/32 + xi*wavelength/16
            y = np.sin(theta*np.pi/180)*outer_radius - 3*wavelength/32 + yi*wavelength/16
    ...
    theta = 35
    x = np.cos(theta*np.pi/180)*(outer_radius - wavelength/8)
    y = np.sin(theta*np.pi/180)*(outer_radius - wavelength/8)
    time_monitor = so.time_monitor(x=x, y = y, z = 5e-9, start_time = 500e-15, min_sampling = 100)
    time_monitor.add_to_sim(sim)

    theta = 45
    x = np.cos(theta*np.pi/180)*(outer_radius - wavelength/16 - 5e-9)
    y = np.sin(theta*np.pi/180)*(outer_radius - wavelength/16 - 5e-9)
    ...
    mesh = so.mesh(x = 0, y = 0, z = 0,
                    xspan = fdtd.xspan, yspan = fdtd.yspan, zspan = thickness + wavelength/2,
    ...
    try:
        ...
        for res_wvl in res_wvls:
            ...
            rect = plt.Rectangle((-outer_radius, -thickness/2), 2*outer_radius, thickness, fill=False, edgecolor='red', linestyle='--')
            ...
            circle = plt.Circle((0, 0), outer_radius, color='red', fill=False, linestyle='--')
            plt.gca().add_artist(circle)
            circle = plt.Circle((0, 0), outer_radius - width, color='red', fill=False, linestyle='--')
            plt.gca().add_artist(circle)
            ...
            plt.axvline(x=outer_radius, color='r', linestyle='--')
            plt.xlabel('r (um)')
            plt.ylabel('|E|^2')
            plt.title(f"99pcnt field decay length: {(outer_radius - r_1d[index])*1e6} um")
            plt.xlim((0, outer_radius*3))
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(output_folder, 'E_1D_' + str(int(XY_DFT_wvls[closest_index]*1e10)) + 'Angstroms.png'))
            plt.close()
            decay_lengths[i] = outer_radius - r_1d[index]
